...
def get_user(user_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, name, house_number, phone, lat, lng FROM users WHERE id = %s",
            (user_id,),
        )
        user_data = cursor.fetchone()
...
def get_alerts_by_group(group_id):
    ...
    try:
        cursor.execute(
            "SELECT user_id FROM user_group WHERE group_id = %s", (group_id,)
        )
        ...
        for alert_data in alerts_data:
            formatted_user = {
                ...
                "location": {
                    "lat": alert_data["user_lat"],
                    "lng": alert_data["user_lng"],
                },
            }
